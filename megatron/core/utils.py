...
class StragglerDetector:
    ...
    def elapsed(self) -> Tuple[float, float, int, int, int, int]:
        ...
        if ls_ev != le_ev:
            ...
            for i in range(ls_ev):
                ...
                e_tm = (self.stop_gemm_tm[i] - self.start_gemm_tm[i]) / 1e6

                delta += max(e_ev, e_tm)

            for i in range(ls_bs):
                b_ev = self.start_data_ev[i].elapsed_time(self.stop_data_ev[i])
                b_tm = (self.stop_data_tm[i] - self.start_data_tm[i]) / 1e6
    ...
    def report(self, total_flops: float = 0.0, log_interval: int = 0) -> bool:
        ...
        if not self._off and total_flops > 0.0 and log_interval > 0:
            elapsed, btime, temp, power, util, clock = self.elapsed()

            ptime = elapsed / (log_interval * 1.0)
            api_flops = total_flops / (log_interval * 1.0)
            apir_flops = api_flops / (
                ptime * 10**9 * self.world
            )
            et_flops = apir_flops / self.amp
    ...
    def _check_toggle(self) -> None:
        ...
        torch.distributed.broadcast(st, 0)
    ...
    def _min_max(
        ...
        if self.rank == 0:
            min_ctime = min(data_list, key=lambda k: k["time"])
            max_ctime = max(data_list, key=lambda k: k["time"])

            min_cbatch = min(data_list, key=lambda k: k["btime"])
            max_cbatch = max(data_list, key=lambda k: k["btime"])

            min_ctemp = min(data_list, key=lambda k: k["temp"])
            max_ctemp = max(data_list, key=lambda k: k["temp"])

            min_cpower = min(data_list, key=lambda k: k["power"])
            max_cpower = max(data_list, key=lambda k: k["power"])

            min_cutil = min(data_list, key=lambda k: k["util"])
            max_cutil = max(data_list, key=lambda k: k["util"])

            min_cclock = min(data_list, key=lambda k: k["clock"])
            max_cclock = max(data_list, key=lambda k: k["clock"])

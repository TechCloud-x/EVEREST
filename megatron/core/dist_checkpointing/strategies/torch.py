...
def sharded_tensor_to_torch_sharded_tensor(
    ...
    if has_flattened_range:
        ...
        offsets_shape = some_sh_ten.data.shape

        local_shards = [
            Shard.from_tensor_and_offsets(
                sh_ten.data, list(sh_ten.global_offset), rank
            )
            for sh_ten in sh_tens
        ]
...
class MCoreSavePlan(SavePlan):
    ...
    mcore_data: Dict[str, Dict[str, Any]] = None
        ...
        if self.use_cached_ckpt_structure:
            if (
                ...
                if save_state_dict_ret[1]:
                    self.cached_global_metadata = save_state_dict_ret[1]
        ...
        for k, tp in metadata.state_dict_metadata.items():
            ...
            if nd_orig_global_shape is None:
                ...
                sharded_metadata[k] = ShardedTensor.from_rank_offsets_flat(
                    ...
                    flat_ten,
                    unflat_ten.shape,
                    flattened_range=slice(0, unflat_ten.numel()),
                ).without_data()

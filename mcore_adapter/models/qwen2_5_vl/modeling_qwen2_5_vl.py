...
class Qwen2_5_VLModel(Qwen2_5_VLBaseModel, ModuleUtilsMixin):
    ...
    def construct_inputs_embeds(
        ...
        for start, end in valid_image_embeds_nums:
            ...
            target_end = min(added_num + embeds_num, needed_image_embeds_num)
            source_end = start + (target_end - added_num)
            if target_end > added_num:
                needed_image_embeds[added_num : target_end] = image_embeds[start:source_end]
                added_num = target_end
        assert added_num == needed_image_embeds_num, f"Mismatch: added_num={added_num}, needed_image_embeds_num={needed_image_embeds_num}"

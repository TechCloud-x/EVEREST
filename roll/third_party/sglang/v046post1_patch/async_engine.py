...
async def consumer(asyncio_queue, consumer_id, llm, request_complete_callback):
    ...
    def process_sglang_output(token_ids, meta_info):
        ...
        logger.info(f"worker_id:{consumer_id} request_id: {meta_info['request_id']} finish!")
        ...
        while True:
            ...
            async with abort_lock:
                if rid_str in abort_rid_set:
                    logger.info(f"request_id: {rid_str} do not running!")
                    abort_rid_set.remove(rid_str)
                    continue

            final_tokens = [[] for _ in range(sampling_params['n'])]
            logger.info(f"worker_id:{consumer_id} request_id: {rid} starting!")
            ...
            async for chunk in generator:
                ...
                async with abort_lock:
                    if rid_str in abort_rid_set:
                        ...
                        logger.info(f"request_id: {rid_str}-{cur_abort_rid} aborting!")
                        llm.tokenizer_manager.abort_request(cur_abort_rid)
                        logger.info(f"request_id: {rid_str}-{cur_abort_rid} abort success!")
                        parallel_sample_num -= 1

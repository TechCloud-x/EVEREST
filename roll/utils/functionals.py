...
def agg_loss(loss_mat: torch.Tensor, loss_mask: torch.Tensor, loss_agg_mode: str,
    ...
    if loss_agg_mode == "token-mean":
        ...
        if weights is None:
            weights = torch.ones(loss_mask.shape[0], device=loss_mask.device)
        loss = (seq_losses * weights * valid_samples).sum() / (valid_samples.sum() + 1e-8)
    elif loss_agg_mode == "seq-mean-token-mean":
        seq_losses = masked_mean(loss_mat, loss_mask, dim=-1)
        seq_losses = seq_losses / (torch.sum(loss_mask, dim=-1) + 1e-8)
        valid_samples = torch.any(loss_mask > 0, dim=-1).float()
        if weights is None:
            weights = torch.ones(loss_mask.shape[0], device=loss_mask.device)
        loss = (seq_losses * weights * valid_samples).sum() / (valid_samples.sum() + 1e-8)
        ...
        if weights is None:
            weights = torch.ones(loss_mask.shape[0], device=loss_mask.device)
        loss = (seq_losses * weights * valid_samples).sum() / loss_mask.shape[-1]
...
def apply_kl_penalty(data: "DataProto", kl_ctrl: AdaptiveKLController, kl_penalty="kl"):
    ...
    current_kl = masked_mean(kld, mask=response_mask, dim=-1)
    current_kl = torch.mean(current_kl, dim=0).item()
...
@torch.no_grad()
def apply_kl_penalty_multi(data: "DataProto", kl_ctrl: AdaptiveKLController, kl_penalty="kl"):
    response_mask = data.batch["response_mask"][:, 1:]
...
def postprocess_generate(
    ...
    prompt = output[:, :prompt_length].clone()
    response = output[:, prompt_length:].clone()
    ...
    new_response_mask = torch.zeros_like(attention_mask)

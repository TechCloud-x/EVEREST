...
class SendBucketManager:
    ...
    def meta_to_dict(meta_infos):
        ...
        All values are converted to serializable types (str/list) so that
        vLLM v1's msgspec-based serializer can handle them.
        """
    ...
    def dict_to_meta(meta_infos):
        for _, meta_info in meta_infos.items():
            ...
            if isinstance(layout, str):
                layout = getattr(torch, layout.replace("torch.", ""))

...
class MegatronInferStrategy(InferenceStrategy):
    ...
    def inner_forward_step(self, loss_func, data_iterator: Iterator[DataProto], model):
        ...
        if "position_ids" in data.batch.keys() and data.batch["position_ids"].dim() == 3:
...
class MegatronTrainStrategy(MegatronInferStrategy, TrainStrategy):
    ...
    def initialize(self, model_provider):
        ...
        steps_per_worker = self.worker_config.training_args.max_steps // self.worker.rank_info.dp_size
        self.worker_config.training_args.max_steps = max(1, steps_per_worker)

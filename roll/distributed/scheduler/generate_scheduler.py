...
class DynamicSamplingScheduler:
    ...
    def get_batch(self, data: DataProto, batch_size: int) -> DataProto:
        ...
        mean_custom_metrics = {}
        if 'custom_metrics' in collect_data[0].meta_info:
            len_collect_data = len(collect_data)
            metrics_list = [batch.meta_info["custom_metrics"] for batch in collect_data]
            mean_custom_metrics = {k:sum([x[k] for x in metrics_list])/len_collect_data for k in metrics_list[0].keys()}


        batch = DataProto.concat(collect_data[: self.batch_size * num_return_sequences])
        batch.meta_info["custom_metrics"] = mean_custom_metrics

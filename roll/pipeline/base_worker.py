...
class ActorWorker(Worker):
    ...
    def loss_func(self, data: DataProto, output_tensor: torch.Tensor):
        ...
        pg_metrics = {
            "actor_train/ppo_ratio_high_clipfrac": clipped_high.mean().detach().item(),
            "actor_train/ppo_ratio_low_clipfrac": clipped_low.mean().detach().item(),
            "actor_train/ppo_ratio_clipfrac": clipped.mean().detach().item(),
            "actor_train/ratio_mean": masked_mean(ratio, response_mask, dim=-1).mean().detach().item(),
            "actor_train/ratio_max": torch.max(ratio * response_mask).detach().item(),
            "actor_train/ratio_min": torch.min(ratio * response_mask + (1 - response_mask) * 1e10).detach().item(),
            "actor_train/clipfrac": agg_loss(loss_mat=torch.lt(surr2, surr1).float(), loss_mask=response_mask,
                                       loss_agg_mode=self.pipeline_config.loss_agg_mode).detach().item(),
            "actor_train/pg_loss": pg_loss.detach().item(),
            "actor_train/kl_loss": kl_loss.detach().item(),
            "actor_train/total_loss": total_loss.detach().item(),
            "actor_train/approxkl": agg_loss(loss_mat=approxkl, loss_mask=response_mask,
                                       loss_agg_mode=self.pipeline_config.loss_agg_mode).detach().item(),
            "actor_train/policykl": agg_loss(loss_mat=policykl, loss_mask=response_mask,
                                       loss_agg_mode=self.pipeline_config.loss_agg_mode).detach().item(),
        }

...
@dataclass
class SocioSegConfig(RLVRConfig):
    seg_infer: WorkerConfig = field(
        default_factory=WorkerConfig,
        metadata={"help": "Configuration for the seg infer role."}
    )

    def __post_init__(self):
        super().__post_init__()
        if self.seg_infer.worker_cls is None:
            self.seg_infer.worker_cls = "roll.pipeline.rlvr.seg_worker.SegWorker"

        self.seg_infer.name = "seg_infer"
